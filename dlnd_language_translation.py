...
def decoding_layer(dec_input, encoder_state,
    ...
    with tf.variable_scope("decode", reuse=True):
        decoding_layer_infer_output=decoding_layer_infer(encoder_state, dec_cell, dec_embeddings, target_vocab_to_int['<GO>'], target_vocab_to_int['<EOS>'],
...
def sentence_to_seq(sentence, vocab_to_int):
    ...
    ans=[]
    for i in words:
        if vocab_to_int.get(i,0):
            ans.append(vocab_to_int[i])
        else:
            ans.append(vocab_to_int['<UNK>'])
    return ans
